...
class REModel(torch.nn.Module):
    def __init__(self, rel_num:int, vocab_size:int) -> None:
        super().__init__()
        self.encoder = transformers.BertModel.from_pretrained('bert-base-uncased')
        self.encoder.resize_token_embeddings(vocab_size)
        self.classifier = torch.nn.Linear(self.encoder.get_input_embeddings().embedding_dim*3, rel_num)
        self.loss_fn = CrossEntropyLoss()
    
    def forward(self, inputs:transformers.BatchEncoding, sub_mask:torch.BoolTensor, obj_mask:torch.BoolTensor, labels:torch.LongTensor = None):
        encoding = self.encoder(**inputs)
        cls_emb = encoding['last_hidden_state'][:, 0, :]
        sub_emb = torch.matmul(torch.swapaxes(encoding['last_hidden_state'], 1, 2), sub_mask.unsqueeze(2).float()).squeeze(2)
        obj_emb = torch.matmul(torch.swapaxes(encoding['last_hidden_state'], 1, 2), obj_mask.unsqueeze(2).float()).squeeze(2)
        pred = self.classifier(torch.concat((cls_emb, sub_emb, obj_emb), dim=1))
        ret = {'logits' : pred}
        if labels is not None:
            ret['loss'] = self.loss_fn(pred, labels)
        return ret